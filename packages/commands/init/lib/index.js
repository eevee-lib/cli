...
const Command = require("@eevee-cli/command");
const log = require("@eevee-cli/log");
const Package = require("@eevee-cli/package");
const { spinnerStart, sleep, execAsync } = require("@eevee-cli/util");
...
class InitCommand extends Command {
                    ...
                    Promise.all(
                        files.map((file) => {
                            ...
                            return new Promise((resolve1, reject1) => {
                                // 进行 <% name% >  projectInfo = { name:"Vue3",version:1.0.0} 替换
    ...
    async getProjectInfo () {
        function isValidName (v) {
            return /^(@[a-zA-Z0-9_]+\/)?[a-zA-Z]+([-][a-zA-Z][a-zA-Z0-9]*|[_][a-zA-Z][a-zA-Z0-9]*|[a-zA-Z0-9])*$/.test(
                v
            );
        }
